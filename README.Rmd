...
- **[Apache Arrow support](https://arrow.apache.org/)**: Polars can consume and produce Arrow data
  often with zero-copy operations. Note that Polars is not built on an official Arrow implementation.
...
It can be used to convert R DataFrames to Polars DataFrames and vice versa,
as well as to integrate with other common R packages.
...
More recent (unstable) version may be installed from the rpolars R-universe repository:
...
And, most of the methods for `polars` objects should be called with the `$` operator.
This means that `polars` queries written in Python and in R are very similar.
Additionally, many functions are intended to match the Python Polars API.
