...
#' @param comm_subplan_elim A logical, indicates tring to cache branching subplans that occur
#' on self-joins or unions.
#' @param comm_subexpr_elim A logical, indicates tring to cache common subexpressions.
...
#'
#' This is similar to a [`<lazyframe>$collect()`][lazyframe__collect] operation,
#' but it overwrites the number of rows read by *every* scan operation.
#' Be aware that `fetch` does not guarantee the final number of rows in the DataFrame.
...
#' if `n_rows` is too small as the join keys may not be present).
#' @inherit lazyframe__collect return
#' @inheritParams lazyframe__collect
#' @param n_rows Scalar integer-ish indicating the number of rows to collect from the data source.
#' @keywords internal
lazyframe___fetch <- function(
  n_rows = 500,
  ...,
  type_coercion = TRUE,
  `_type_check` = TRUE,
