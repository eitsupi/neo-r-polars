...
test_that("`[` operator works to subset columns only", {
  ...
  ### Indices
  expect_identical(test[1], pl$LazyFrame(a = 1:3))
  expect_warning(
    expect_identical(test[1, drop = TRUE], pl$LazyFrame(a = 1:3)),
    "`drop` argument ignored for subsetting a frame",
  )
  expect_identical(test[, 1, drop = TRUE], pl$Series("a", 1:3))
  expect_identical(test[1:2], pl$LazyFrame(a = 1:3, b = 4:6))
  expect_identical(test[, 1:2, drop = TRUE], pl$LazyFrame(a = 1:3, b = 4:6))
  expect_identical(test[, -2:-1], pl$LazyFrame(c = 7:9))
  ...
  ### Column names
  expect_identical(test["a"], pl$LazyFrame(a = 1:3))
  expect_warning(
    expect_identical(test["a", drop = TRUE], pl$LazyFrame(a = 1:3)),
    "`drop` argument ignored for subsetting a frame",
  )
  expect_identical(test[, "a", drop = TRUE], pl$Series("a", 1:3))
  expect_identical(test[c("a", "b")], pl$LazyFrame(a = 1:3, b = 4:6))
  expect_identical(test[, c("a", "b"), drop = TRUE], pl$LazyFrame(a = 1:3, b = 4:6))
  expect_error(
    test[c("a", "foo")],
    "not found: foo"
  )
  ...
  expect_identical(test[c(TRUE, TRUE, FALSE)], pl$LazyFrame(a = 1:3, b = 4:6))
  expect_error(
    test[c(TRUE, FALSE)],
...
test_that("`[` operator works to subset rows only", {
  ...
  expect_identical(
    test[1, ],
    pl$LazyFrame(a = 1L, b = 4L, c = 7L)
  )
  expect_identical(
    test[1:2, ],
    pl$LazyFrame(a = 1:2, b = 4:5, c = 7:8)
  )
  # TODO
  # expect_identical(
  #   test[c(1, 10), ],
  #   pl$LazyFrame(a = c(1L, NA_integer_), b = c(4L, NA_integer_), c = c(7L, NA_integer_))
  # )
  expect_identical(
    test[-2:-1, ],
    pl$LazyFrame(a = 3L, b = 6L, c = 9L)
  )
  expect_snapshot(test[-2:1, ], error = TRUE)
  expect_snapshot(test[1:-2, ], error = TRUE)
  expect_snapshot(test[1.5, ], error = TRUE)
  ...
  # expect_identical(
  #   test["foo", ],
  #   pl$LazyFrame(a = NA_integer_, b = NA_integer_, c = NA_integer_)
  # )
  ...
  # expect_identical(
  #   test[NA, ],
  #   pl$LazyFrame(a = rep(NA, 3), b = rep(NA, 3), c = rep(NA, 3))
  # )
  # expect_identical(
  #   test[NA_integer_, ],
  #   pl$LazyFrame(a = NA, b = NA, c = NA)
  # )
  expect_error(
    test[c(TRUE, FALSE), ],
    "must be size 1 or 3, not 2"
...
test_that("`[` operator works to subset both rows and columns", {
  test <- pl$LazyFrame(a = 1:3, b = 4:6, c = 7:9)
  expect_identical(test[1:2, "a"], pl$LazyFrame(a = 1:2))
  expect_identical(test[TRUE, "a"], pl$LazyFrame(a = 1:3))
})
